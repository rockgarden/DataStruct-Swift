...
//  SortTests
//
//  Created by wangkan on 2017/7/24.
...
class SortTests: XCTestCase {
    ...
    func testLinuxTestSuiteIncludesAllTests() {
            ...
            #if swift(>=4.0)
                let darwinTestCount = SortTests.defaultTestSuite.testCaseCount
            #else
                let darwinTestCount = Int(SortTests.defaultTestSuite().testCaseCount)
            #endif
            let linuxTestCount = SortTests.allTests.count
...
extension SortTests {
    static var allTests : [(String, (SortTests) -> () throws -> Void)] {
        return [
